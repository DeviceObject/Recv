...
int my_encrypt(char *input_string, char **encrypt_string)
{
	AES_KEY aes;
	...
	*encrypt_string = (char*)calloc(len, sizeof(unsigned char)); 
...
void my_decrypt(char *encrypt_string, char **decrypt_string,int len)
{
	unsigned char key[AES_BLOCK_SIZE]; // AES_BLOCK_SIZE = 16
	unsigned char iv[AES_BLOCK_SIZE]; // init vector
	AES_KEY aes;
	...
	*decrypt_string = (char*)calloc(len, sizeof(unsigned char));
